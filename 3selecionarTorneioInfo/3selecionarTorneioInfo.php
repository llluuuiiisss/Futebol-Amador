...
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" type="text/css" href="3selecionarTorneioInfo.css">
...
<div id="navigation">
    <img src="../3selecionarEquipa/user.png" class="playerImg">
    ...
    <ul>
        ...
        <li> <a href="<?php
            ...
            while($row = mysqli_fetch_assoc($result)){
                if($row['username'] == $_SESSION['login_user']){
                    if($row['admin']==0){
                        echo "3selecionarTorneioInfo.php";
                    }else{
        ...
        <table id="tournments" class="tournments">
            ...
            if ($result -> num_rows > 0) {
                while ($row = $result -> fetch_assoc()){
                    echo "<tr><td>{$row['nome']}</td> <td> Gestor: {$row['criador']}</td> <td>Início :{$row['start']}</td> <td>Fim: {$row['end']}</td></tr>";
                }
